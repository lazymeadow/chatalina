...
internal class AsciiTest {
    fun String.getExpectedEmojiVal() = emojiManager.emojiData.find { it.hexcode == this }?.emoji
    fun String.formatEmojis(vararg args: String) = this.format(*args.map { it.getExpectedEmojiVal() }.toTypedArray())
    ...
    fun testAlone() {
        ...
        emojiManager.asciiMap.forEach { (ascii, hex) ->
            ...
            if (hex.getExpectedEmojiVal() != result) errors.add(ascii)
        }
