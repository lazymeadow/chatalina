...
export class SoundManager extends LoggingClass {
    ...
    playSampleSound(soundSet, atVolume) {
        let sampleSound = $('#sample')
        if (!sampleSound.length) {
            sampleSound = $('<audio>', { id: 'sample' }).attr('type', 'audio/mpeg')
            $('body').append(sampleSound)
        }
        sampleSound.attr('src', `${this._soundAssetUrl}/${soundSet}/message-receive.wav`).prop('volume', atVolume / 100)
        sampleSound[0].play()
    }
