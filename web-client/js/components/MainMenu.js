...
export class MainMenu extends LoggingClass {
    constructor(chatClient, allowedItems) {
        ...
        if (mainMenu.next('.popout-menu').length) {
            this._menuElement = mainMenu.next('.popout-menu')
        } else {
            this._menuElement = mainMenu
        }
    ...
    _new_menu_item(title, icon, clickHandler) {
        if (this._menuElement.hasClass('mobile')) {
            ...
            return $('<div>', { title })
                .append($('<span>').addClass(`${icon[0]} fa-fw fa-${icon[1]}`))
                .append($('<span>').addClass('list-content').text(title))
                .click(event => {
                    event.stopPropagation()
                    clickHandler()
                })
            ...
            () => {
                ...
                return new Modal({
                    ...
                    buttonClickHandler: () => {
                        ...
                        const serverChangesMade = Object.keys(serverChanges).length > 0

                        if (!clientChangesMade && !serverChangesMade) {
                            ...
                            if (serverChangesMade) {
                                this._chatClient.updateSettings(serverChanges)
